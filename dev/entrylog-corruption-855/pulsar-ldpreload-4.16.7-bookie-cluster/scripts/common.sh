...
BOOKIE_COUNT="${BOOKIE_COUNT:-3}"
MANAGED_LEDGER_DEFAULT_ENSEMBLE_SIZE="${MANAGED_LEDGER_DEFAULT_ENSEMBLE_SIZE:-3}"
MANAGED_LEDGER_DEFAULT_WRITE_QUORUM="${MANAGED_LEDGER_DEFAULT_WRITE_QUORUM:-3}"
MANAGED_LEDGER_DEFAULT_ACK_QUORUM="${MANAGED_LEDGER_DEFAULT_ACK_QUORUM:-2}"
...
BK_DIST_TARBALL="${BK_DIST_TARBALL:-/tmp/bookkeeper-4.16.7-clean/bookkeeper-dist/server/target/bookkeeper-server-${BK_VERSION}-bin.tar.gz}"
...
LD_PRELOAD_FAULT_TOOL_DIR="${LD_PRELOAD_FAULT_TOOL_DIR:-${REPO_ROOT}/dev/entrylog-corruption-855/tools/ldpreload-entrylog-fault}"
LD_PRELOAD_FAULT_SOURCE_LIB="${LD_PRELOAD_FAULT_SOURCE_LIB:-${LD_PRELOAD_FAULT_TOOL_DIR}/libbk_entrylog_fault.so}"
LD_PRELOAD_FAULT_LIB="${LD_PRELOAD_FAULT_LIB:-${RUNTIME_DIR}/fault/libbk_entrylog_fault.so}"
...
require_prepared() {
  ...
  if [[ ! -f "${LD_PRELOAD_FAULT_LIB}" ]]; then
    echo "LD_PRELOAD fault library is not prepared: ${LD_PRELOAD_FAULT_LIB}" >&2
    echo "Run scripts/prepare-local-cluster.sh first." >&2
    exit 1
  fi
}
