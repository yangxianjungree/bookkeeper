...
if "${jar_cmd}" tf "${bk_server_jar}" | grep -Fxq "${failpoint_class}"; then
  echo "BookKeeper ${BK_VERSION} jar unexpectedly contains failpoint class: ${failpoint_class}" >&2
  exit 1
fi

if [[ ! -f "${LD_PRELOAD_FAULT_TOOL_DIR}/entrylog_fault.c" ]]; then
  echo "Missing LD_PRELOAD fault injector source: ${LD_PRELOAD_FAULT_TOOL_DIR}/entrylog_fault.c" >&2
  exit 1
fi

make -C "${LD_PRELOAD_FAULT_TOOL_DIR}"
if [[ ! -f "${LD_PRELOAD_FAULT_SOURCE_LIB}" ]]; then
  echo "Missing built LD_PRELOAD fault injector: ${LD_PRELOAD_FAULT_SOURCE_LIB}" >&2
  exit 1
fi
mkdir -p "$(dirname "${LD_PRELOAD_FAULT_LIB}")"
cp "${LD_PRELOAD_FAULT_SOURCE_LIB}" "${LD_PRELOAD_FAULT_LIB}"

cp "${PULSAR_HOME}/conf/zookeeper.conf" "${CONF_DIR}/zookeeper.conf"
cat >> "${CONF_DIR}/zookeeper.conf" <<EOF

# entrylog partial flush LD_PRELOAD 4.16.7 bookie harness overrides
dataDir=${DATA_DIR}/zk
clientPort=2181
...
# Minimal unmodified BookKeeper ${BK_VERSION} bookie config for the entrylog partial flush LD_PRELOAD harness.
bookiePort=${port}
...
bookieCount=${BOOKIE_COUNT}
managedLedgerDefaultEnsembleSize=${MANAGED_LEDGER_DEFAULT_ENSEMBLE_SIZE}
managedLedgerDefaultWriteQuorum=${MANAGED_LEDGER_DEFAULT_WRITE_QUORUM}
managedLedgerDefaultAckQuorum=${MANAGED_LEDGER_DEFAULT_ACK_QUORUM}
bookieDistribution=${BK_ARCHIVE}
bookieServerJar=${bk_server_jar}
failpointClassAbsent=${failpoint_class}
ldPreloadFaultSource=${LD_PRELOAD_FAULT_TOOL_DIR}/entrylog_fault.c
ldPreloadFaultLib=${LD_PRELOAD_FAULT_LIB}
EOF

echo "Prepared Pulsar ${PULSAR_VERSION} with unmodified external BookKeeper ${BK_VERSION} bookies under ${RUNTIME_DIR}"
echo "Verified failpoint class is absent from ${bk_server_jar}"
echo "Prepared LD_PRELOAD fault injector at ${LD_PRELOAD_FAULT_LIB}"
